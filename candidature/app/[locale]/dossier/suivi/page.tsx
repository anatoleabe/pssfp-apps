...
export default async function SuiviPage(): Promise<JSX.Element> {
  const tsp = await getTranslations('dossier.suiviPage');
  ...
  if (!result.ok) {
    ...
    return (
      <div className="mx-auto max-w-3xl px-6 py-16">
        <h1 className="font-heading text-3xl font-bold text-[#4A2E67]">{tsp('breadcrumbCurrent')}</h1>
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10 md:py-16">
      <header className="mb-8 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-3xl font-bold text-[#4A2E67]">{tsp('title')}</h1>
          <p className="mt-1 text-sm text-[#666]">
            {tsp('fileNumber')} <span className="font-mono">{candidature.numero_dossier}</span>
          </p>
        </div>
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <h2 className="mb-6 font-heading text-lg font-bold text-[#4A2E67]">{tsp('steps')}</h2>
        <StatusTimeline candidature={candidature} />
      </section>

      {canWithdraw && (
        <section className="mt-8 rounded-lg border border-red-200 bg-red-50 p-6">
          <h2 className="font-heading text-base font-bold text-red-700">{tsp('dangerZone')}</h2>
