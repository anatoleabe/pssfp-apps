...
server {
    ...
    # Proxy vers Next.js frontend (port 6000 local)
    location / {
...
Vhosts similaires pour `bibliotheque.pssfp.net` (port 6001), `candidature.pssfp.net` (port 6002), `api.pssfp.net` (PHP-FPM via fastcgi). Variations principales sur :
...
module.exports = {
  apps: [
    { name: 'pssfp-frontend', cwd: '/var/www/pssfp/frontend', script: 'pnpm', args: 'start', env: { PORT: 6000, NODE_ENV: 'production' } },
    { name: 'pssfp-library', cwd: '/var/www/pssfp/library', script: 'pnpm', args: 'start', env: { PORT: 6001, NODE_ENV: 'production' } },
    { name: 'pssfp-candidature', cwd: '/var/www/pssfp/candidature', script: 'pnpm', args: 'start', env: { PORT: 6002, NODE_ENV: 'production' } },
  ],
};
```
